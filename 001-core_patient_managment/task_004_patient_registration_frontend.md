...
### Goal
**What to Build:** Complete React-based patient registration interface with form validation, multi-language support, photo capture capabilities, and seamless integration with the backend patient registration API.

**Why:** Provide healthcare staff with an intuitive, efficient interface for registering new patients, replacing manual paper-based processes and ensuring data accuracy.
...
```yaml
performance:
  - requirement: "Form interaction responsiveness"
    target: "< 100ms response time for validation feedback"
    
  - requirement: "Image processing performance"
    target: "< 2 seconds for photo capture and processing"
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              {t('registration.step_progress', {
                ...
                total: STEPS.length,
                step: t(STEPS[currentStep].titleKey)
              })}
            </p>
          </div>
        </CardHeader>
  ...
  return (
    <div className="space-y-6">
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel>{t('fields.middle_name')}</FormLabel>
            <FormControl>
              <Input
                {...field}
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel>{t('fields.preferred_contact_method')}</FormLabel>
            <Select onValueChange={field.onChange} defaultValue={field.value}>
              <FormControl>
                <SelectTrigger>
                  <SelectValue />
              ...
              <SelectContent>
                <SelectItem value="phone">{t('contact_method.phone')}</SelectItem>
                <SelectItem value="email">{t('contact_method.email')}</SelectItem>
                <SelectItem value="sms">{t('contact_method.sms')}</SelectItem>
              </SelectContent>
            </Select>
            <FormMessage />
          </FormItem>
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-medium">{t('emergency_contacts.title')}</h3>
          <p className="text-sm text-muted-foreground">
        ...
        <Card key={field.id}>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
            <CardTitle className="text-base">
              {t('emergency_contacts.contact_number', { number: index + 1 })}
            </CardTitle>
            <Button
              type="button"
              variant="ghost"
...
- [ ] Photo capture functionality works with camera and file upload
- [ ] Arabic and French language support with proper RTL layout
